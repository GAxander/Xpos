...
import { Utensils, Users, Square, Save, Move, Clock, LockKeyhole, Wallet } from 'lucide-react';
...
export default function Home() {
  ...
  const handleStop = async (e: DraggableEvent, data: DraggableData, tableId: string) => {
    ...
    const token = localStorage.getItem('pos_token');
    try {
      await fetch(`http://localhost:3000/api/v1/floor/table/${tableId}`, {
        method: 'PATCH',
        headers: { 
  ...
  if (!isShiftOpen) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 font-sans">
        <div className="bg-white p-10 rounded-3xl shadow-xl max-w-md w-full text-center border border-slate-100 animate-in zoom-in-95 duration-300">
          <div className="bg-rose-50 w-24 h-24 rounded-full flex items-center justify-center mx-auto mb-6 border-4 border-rose-100">
            <LockKeyhole className="w-10 h-10 text-rose-500" />
          </div>
          
          <h1 className="text-2xl font-black text-slate-800 mb-2">Caja Cerrada</h1>
          ...
          <button 
            ...
            className="w-full py-4 font-black text-white bg-emerald-600 hover:bg-emerald-700 rounded-2xl flex justify-center items-center gap-2 transition-all shadow-lg shadow-emerald-200 active:scale-95"
          >
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-6 md:p-8 font-sans">
      <header className="flex justify-between items-center mb-8 bg-white p-5 rounded-3xl shadow-sm border border-slate-100">
        <div>
          <h1 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-3">
            <Utensils className="text-emerald-600 w-7 h-7" /> 
            Plano de Sala
          </h1>
          <p className="text-slate-500 font-medium mt-0.5 text-sm">Xpos Cloud - Huanchaco Vista al Mar</p>
        </div>
        <button 
          onClick={() => setIsEditMode(!isEditMode)}
          className={`flex items-center gap-2 px-5 py-2.5 rounded-2xl font-bold transition-all active:scale-95 border-2 
            ${isEditMode 
        ...
        >
          ...
          {isEditMode ? 'Guardar Cambios' : 'Modificar Plano'}
        </button>
      </header>

      {isEditMode && (
        <div className="mb-6 p-4 bg-blue-50 border border-blue-200 rounded-2xl text-blue-800 text-sm font-medium flex items-center gap-3 shadow-sm">
      ...
      <div className="space-y-6">
        {zones.map(zone => (
          <section key={zone.id} className="bg-white p-5 rounded-3xl shadow-sm border border-slate-100">
            ...
            <div className="relative w-full min-h-[500px] bg-[url('https://transparenttextures.com/patterns/cubes.png')] bg-slate-50/50 rounded-2xl border-2 border-dashed border-slate-200 overflow-hidden">
              
              {zone.tables.length === 0 && (
                <div className="absolute inset-0 flex flex-col gap-3 items-center justify-center text-slate-400 font-medium">
